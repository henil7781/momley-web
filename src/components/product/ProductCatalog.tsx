...
import { Search, X, Beaker, Thermometer, ShieldCheck } from 'lucide-react';
...
const ProductCatalog: React.FC<ProductCatalogProps> = ({ searchTerm, setSearchTerm }) => {
    ...
    // Simulate initial loading for "Speed of Trust"
    useEffect(() => {
    ...
    // Handle Product Selection (Tracking for "Recently Viewed")
            ...
            <div className="max-w-7xl mx-auto space-y-12">
                ...
                <div className="bg-white/40 backdrop-blur-3xl border border-white/20 rounded-3xl p-5 shadow-2xl shadow-pink-500/5 flex flex-col lg:flex-row items-center gap-5 sticky top-28 z-[50]">
                    ...
                    <div className="relative flex-1 w-full group">
                        ...
                        <input
                            type="text"
                            placeholder="Search product, category, composition..."
                            value={searchTerm}
                    ...
                    {/* Filter Selectors using Custom DialogDropdown */}
                ...
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <AnimatePresence>
                        {activeCategory !== 'All' && (
                            <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="px-3 py-1 bg-brand-pink/10 text-brand-pink text-[10px] font-black rounded-full flex items-center gap-2">
                                {activeCategory} <X className="w-3 h-3 cursor-pointer" onClick={() => setActiveCategory('All')} />
                            </motion.span>
                        )}
                        {activeType !== 'All' && (
                            <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="px-3 py-1 bg-blue-50 text-blue-600 text-[10px] font-black rounded-full flex items-center gap-2">
                                {activeType} <X className="w-3 h-3 cursor-pointer" onClick={() => setActiveType('All')} />
                            </motion.span>
                        )}
                    </AnimatePresence>
                </div>
                    ...
                    <AnimatePresence mode="popLayout">
                        {isLoading ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-40 text-center space-y-4">
                                ...
                                <p className="text-slate-500 max-w-xs mx-auto">Try refining your search or changing the therapeutic category.</p>
                            </div>
                        )}
                    </AnimatePresence>
                </main>
                    ...
                    <>
                        ...
                        >
                            ...
                            <div className="space-y-12">
                                <div className="pt-8">
                                    <span className="text-brand-pink font-black text-[10px] uppercase tracking-[0.3em]">Technical Monograph</span>
                                ...
                                <button className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-brand-pink transition-all">
